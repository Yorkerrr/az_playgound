...
bash -c "for i in {1..10}; do curl -s curl http://${ISTIOD_REMOTE_EP}/hello; done"
...
kubectl exec --context="${MAIN_CLUSTER_CTX}" -n sample -c sleep-primary \
    ...
    -- sh -c 'for i in `seq 1 10`; do curl -s helloworld-secondary.sample.global:5000/hello; done'
