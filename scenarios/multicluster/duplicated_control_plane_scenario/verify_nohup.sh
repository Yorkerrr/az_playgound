...
echo "Verify Primary cluster Sleep to Secondary cluster via helloworld-secondary.sample.global:5000/hello"
...
nohup kubectl exec --context="${MAIN_CLUSTER_CTX}" -n sample -c sleep-primary \
    ...
    -- sh -c 'while true; do echo "sleep-primary to helloworld-secondary"; curl -s helloworld-secondary.sample.global:5000/hello; done' &
...
nohup kubectl exec --context="${REMOTE_CLUSTER_CTX}" -n sample -c sleep-secondary \
    ...
    -- sh -c 'while true; do echo "sleep-secondary to helloworld-primary"; curl -s helloworld-primary.sample.global:5000/hello; done' &